...
- 真实用户环境：固定发布 commit 已安装到真实 Web profile，并通过一次只读显式调用烟测。
...
隔离 Runtime 与 GitHub 固定 commit 安装结论只适用于对应的固定 Harness 身份；真实 Web consumer 结论只覆盖 `0.1.0-rc.6`、上述固定适配器 commit 和一个空的非 Git 工作区。非空 Git 项目中的实际治理效果仍未验证。
