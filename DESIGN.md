...
- 项目治理：已建立。
- 适配器源码：`0.1.2` 未发布候选已同步到 Swift Cycle commit `f383157fce7d179f29de867605d16e01b64366c8`，本地自动化测试已通过。
- 打包制品：`0.1.2` 候选的 13 文件 dry-run 合同及临时解包后完整性验证已通过。
...
- `0.1.2` Runtime/consumer：尚未安装、发布或执行模型调用，保持 `NOT_VERIFIED`。
...
适配器携带只读的上游快照，通过 DeepSeek Harness 的 `ctx.skills.register()` 注册 `swift-cycle`。上游身份和文件由 `upstream.lock.json` 及 SHA-256 校验约束；Harness 专用调用策略位于快照之外。当前 `0.1.2` 候选锁定未打 tag 的 commit `f383157fce7d179f29de867605d16e01b64366c8`，并携带 canonical Skill、host metadata、两份 reference 和九份可选模板。
...
- 当前源码候选基线：`dsh-plugin-swift-cycle@0.1.2`、上游 commit `f383157fce7d179f29de867605d16e01b64366c8`、payload SHA-256 `fff7094f40c291cc9e03aa96ad271ef110229aba2fb7afa322473949043e4c19`；仅源码与本地 pack 候选已验证。
...
隔离 Runtime 与各分发身份的安装结论只适用于对应的固定 Harness 和适配器身份；真实 Web consumer 结论只覆盖 `0.1.0-rc.6`、GitHub `v0.1.0` 固定适配器 commit 在一个空的非 Git 工作区和本仓库中的只读调用，以及 npm `v0.1.1` 在本仓库上述 HEAD 的一次只读调用。`0.1.2` 候选的隔离 Runtime、真实 profile、consumer、发布和生产治理均未验证。
