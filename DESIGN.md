...
- 项目治理：已建立。
- 适配器源码：`0.1.3` 锁定 Swift Cycle `v1.3.0` / commit `f6645dc2768132e48bb936147f40a954855e5ccf`；18/18 自动化测试和显式上游 checkout 比对已通过，发布 commit 为 `0c491852234be515083590802a4404476f701028`。
...
- 权威治理内容：Swift Cycle 上游仓库；当前适配器载荷以 `v1.3.0` 和精确 commit 为身份。
...
隔离 Runtime 与各分发身份的安装结论只适用于对应的固定 Harness 和适配器身份；真实 Web consumer 结论只覆盖 `0.1.0-rc.6`、GitHub `v0.1.0` 固定适配器 commit 在一个空的非 Git 工作区和本仓库中的只读调用，以及 npm `v0.1.1` 在本仓库上述 HEAD 的一次只读调用。`0.1.3` 的公共发布已验证；隔离 Runtime、真实 profile、consumer 和生产治理仍未验证。
